...
function Capture() {
  ...
  return (
    <>
      <PageHeader eyebrow="New moment" title="Capture a memory" subtitle="Type, speak, or drop an image. The AI takes it from there." />
      ...
      <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
        <div className="glass-strong rounded-3xl p-7">
          ...
          <div className="mt-7">
            <div className="mb-3 text-xs uppercase tracking-widest text-muted-foreground">How did it feel?</div>
              ...
              {EMOTIONS.map(em => {
                ...
                return (
                  <button key={em} onClick={() => setEmotion(em)}
                    ...
                    {active && <span className={`absolute inset-0 bg-gradient-to-r ${EMOTION_COLORS[em]}`} />}
                    {!active && <span className="absolute inset-0 border border-white/10 bg-white/[0.04] rounded-full" />}
        ...
        <div className="flex flex-col items-center text-center">
          <AIOrb size={200} floating message={analyzing ? "Reading the feeling…" : "I'm listening."} />
        </div>
      </div>
        ...
        {analyzing && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              <p className="mt-2 text-sm text-muted-foreground">Detecting themes, emotions, and connections.</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
