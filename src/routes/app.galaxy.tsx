  ...
  return (
    <>
      <PageHeader
        ...
        title="Your inner cosmos"
        subtitle="Each star is a moment. Each thread, an emotional connection."
      />

      <div ref={containerRef} className="glass relative h-[70vh] overflow-hidden rounded-3xl">
        <div className="absolute inset-0 grid-cosmic opacity-25" />
        {/* Drifting nebulae */}
        <div
          className="absolute -top-32 -left-32 h-96 w-96 rounded-full opacity-30 blur-3xl animate-drift"
          style={{ background: "radial-gradient(circle, var(--cyan-glow), transparent 60%)" }}
        />
        <div
          className="absolute -bottom-32 -right-32 h-96 w-96 rounded-full opacity-30 blur-3xl animate-drift"
          style={{
        ...
        {active && (
          ...
          >
            ...
            >
              ...
              <div
                className={`mb-4 inline-block rounded-full bg-gradient-to-r ${EMOTION_COLORS[active.emotion]} px-3 py-1 text-xs font-medium text-black/80`}
              >
                {active.emotion}
              </div>
