...
function Replay() {
  ...
  return (
    <>
      <PageHeader
        eyebrow="Cinematic Replay"
        title="On this thread"
        subtitle="AI-curated moments, restaged with light."
      />
      ...
      <div className="relative">
        <AnimatePresence mode="wait">
          ...
          >
            <div
              className={`absolute -top-40 -right-40 h-[28rem] w-[28rem] rounded-full bg-gradient-to-br ${grad} opacity-30 blur-3xl`}
            />
            <div
              className={`absolute -bottom-40 -left-40 h-[28rem] w-[28rem] rounded-full bg-gradient-to-br ${grad} opacity-20 blur-3xl`}
            />
            ...
            <div className="relative grid gap-12 md:grid-cols-[1fr_auto] md:items-center">
              <div>
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  <span
                    className={`inline-block rounded-full bg-gradient-to-r ${grad} px-3 py-1 text-xs font-medium text-black/80`}
                  >
                    {m.emotion}
                  </span>
                  {m.tags.map((t) => (
      ...
      <h2 className="mt-16 mb-5 font-display text-2xl font-semibold tracking-tight text-aurora">
        Related threads
      </h2>
      <div className="grid gap-4 md:grid-cols-3">
        {items
          .filter((x) => x.emotion === m.emotion && x.id !== m.id)
          .slice(0, 3)
          .map((rel) => (
            <button
              key={rel.id}
              onClick={() => setI(items.indexOf(rel))}
              className="glass group rounded-2xl p-5 text-left transition hover:bg-white/[0.07]"
            >
              <div
                className={`mb-2 inline-block rounded-full bg-gradient-to-r ${EMOTION_COLORS[rel.emotion]} px-2.5 py-0.5 text-[10px] font-medium text-black/80`}
              >
                {rel.emotion}
              </div>
